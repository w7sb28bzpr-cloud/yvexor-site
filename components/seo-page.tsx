...
import { PageData } from "./structured-data";
...
export function SeoPage({page}:{page:SeoPageData}){
  ...
  return <><PageData path={`/${page.slug}/`} title={page.title} description={page.description} service={page.service?page.title:undefined} faq={page.kind==="faq"?page.sections:page.faq}/><SiteHeader root/><main className={`seo-page${automation?" automation-page":""}${detail?" detail-page":""}`} id="contenu">
    <div className="detail-back-wrap"><BackLink/></div>
    <section className="seo-hero" id="accueil"><div className="seo-orbit" aria-hidden="true"/><nav className="seo-breadcrumb" aria-label="Fil d’Ariane"><a href="/">Accueil</a><span aria-hidden="true">/</span><span aria-current="page">{page.title}</span></nav><p className="eyebrow">{page.eyebrow}</p><h1>{page.title}</h1><p className="seo-lead">{page.intro}</p><div className="hero-actions"><a className="action-link primary" href="https://client.yvexor.com/">{page.cta??"Présenter mon projet"}</a><a className="phone" href="/#solutions">Toutes les solutions</a></div></section>
    ...
    {page.faq?.length&&<section className="seo-faq" id="questions"><h2>Questions sur {page.eyebrow.toLowerCase()}</h2>{page.faq.map(q=><article key={q.title}><h3>{q.title}</h3><p>{q.text}</p></article>)}</section>}
    ...
    <aside className="seo-related" id="exemples"><p className="eyebrow">Continuer votre recherche</p><div>{page.related.map(slug=><a href={`/${slug}/`} key={slug}>{seoPageBySlug[slug]?.title??slug.replaceAll("-"," ")}<b aria-hidden="true">→</b></a>)}</div></aside>
    <section className="seo-final" id="projet"><BrandLogo compact/><h2>Votre besoin d’abord. La technologie ensuite.</h2><p>Nous cherchons la base, le périmètre et la première étape les plus cohérents.</p><a className="action-link primary" href="https://client.yvexor.com/">Rejoignez-nous pour échanger</a></section>
  </main><SiteFooter/><MobileTabBar root/></>;
}
