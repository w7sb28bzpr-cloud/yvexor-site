...
function Orb() {
  ...
  useEffect(() => {
    ...
    const geo=new THREE.IcosahedronGeometry(1.25,4); const mat=new THREE.MeshBasicMaterial({color:0x31d7ff,wireframe:true,transparent:true,opacity:.3}); group.add(new THREE.Mesh(geo,mat));
    const count=220, pos=new Float32Array(count*3); for(let i=0;i<count*3;i++) pos[i]=(Math.random()-.5)*5;
    const pgeo=new THREE.BufferGeometry(); pgeo.setAttribute("position",new THREE.BufferAttribute(pos,3)); group.add(new THREE.Points(pgeo,new THREE.PointsMaterial({color:0x1479ff,size:.018,transparent:true,opacity:.8})));
