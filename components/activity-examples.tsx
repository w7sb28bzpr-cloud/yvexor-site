...
const activities = [{name:"Commerce",href:"/logiciel-caisse-marseille/"},{name:"Restaurant",href:"/solutions-restaurants/"},{name:"Garage",href:"/logiciel-garage/"},{name:"Hôtel",href:"/solutions-hotels/"},{name:"Immobilier",href:"/domotique-batiments/"},{name:"Entreprise",href:"/automatisation-entreprise/"}];
const projects = [
...
export function ActivityExamples() {
  return <section className="section sectors" id="exemples">
    ...
    <div className="sector-rail">{activities.map((activity, index) => <a href={activity.href} key={activity.name}><span>0{index + 1}</span><h3>{activity.name}</h3><ChevronIcon/></a>)}</div>
